...
<div class="shop_goods_bd clear">
    ...
    <div class="shop_goods_show">
        <div class="shop_goods_show_left">
            ...
            <div id="preview">
                <div class=jqzoom id="spec-n1" onClick="window.open('/')">
                    <!-- 显示的大图 -->
                    <IMG height="350" src="${goods.cover}" jqimg="${goods.cover}" width="350">
                </div>
                <div id="spec-n5">
            ...
            <script type="text/javascript">
                ...
                var start = new Date('${goods.seckill.startTime?datetime}');

                $.ajax({
                    type: "GET",
                    async: "false",
                    url: "http://localhost:60000/seckill/getNowTime",
                    success: function (time) {
                        now = new Date(time);
                    }
                });

                var countdown = now.getTime() - start.getTime();

                var hour = parseInt(countdown / 1000 / 60 / 60);
                var minute = parseInt(countdown / 1000 / 60 % 60);
                var second = parseInt(countdown / 1000 % 60);
                var millisecond = parseInt(countdown % 1000);

                var html="";
                if (10> hour)
                    html = "0"+hour + ":" + minute + ":" + second;
                else
        ...
        <div class="shop_goods_show_right">
            <ul>
                ...
                <li>
                    <label>价格：</label>
                    <span><strong><del>${goods.price?string("￥#,###.##")}</del></strong>元</span>
                </li>
                <li>
                    <label>秒杀价：</label>
                    <span>${goods.seckill.sPrice?string("￥#,###.##")}</span>
                </li>
                <li>
                    <label>剩余库存：</label>
                    <span>${goods.seckill.sGnum}件</span>
                </li>
                <li>
                    <label>评价：</label>
                    <span>0条评论</span>
                ...
                <li style="padding:20px 0;">
                    <label>&nbsp;</label>
                    <span id="countdown" style="font-size: 40px;color: red;">00:00:00</span>
                    <label>&nbsp;</label>
                    <span><button disabled="disabled">即将开始</button></span>
                    <#--                    <span><a class="goods_sub goods_sub_gou" onclick="addcar()" >加入购物车</a></span>-->
                </li>
            </ul>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function addcar() {
            ...
            //加入购物车
            location.href = "http://192.168.252.1:60000/car/addCar?gid=" + gid + "&gnum=" + gnumber;
        }
